...
import { AVATAR_PRESETS } from "../_data/avatarPresets";
import type { AvatarState } from "../types";

type Props = {
  state: AvatarState;
  applyPreset: (presetState: Partial<AvatarState>) => void;
};
...
export default function PresetsSection({ state, applyPreset }: Props) {
  ...
  const families = useMemo(
    () => Array.from(new Set(AVATAR_PRESETS.map((preset) => preset.family))),
    [],
  );
  const archetypes = useMemo(
    () => Array.from(new Set(AVATAR_PRESETS.map((preset) => preset.archetype))),
    [],
  );
  const sizes = useMemo(
    () => Array.from(new Set(AVATAR_PRESETS.map((preset) => preset.size))),
    [],
  );
  ...
  return (
    <div className="space-y-4">
      <Section
        title="Presets"
        subtitle={`${AVATAR_PRESETS.length} editable starting points built from the current avatar system.`}
      >
        <div className="space-y-4">
